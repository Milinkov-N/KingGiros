...
import { ErrType } from 'src/hooks/useCheckout'
...
export default function Checkout() {
  ...
  return (
    <Layout title='Оформление заказа'>
      <Container>
        <Error show={showError} onClose={() => setShowError(false)} />
        {cartState.items.length > 0 && (
          <Form className={styles.form} onSubmit={handleSubmit(submitSettings)}>
            <ContactInfo phoneIsCorrect={phoneIsIncorrect} />
            <OrderDetails />
          </Form>
        )}
      </Container>
